...
export class OrphanDetector {
  ...
  private isOrphaned(entry: PortEntry): boolean {
  ...
  private buildReason(entry: PortEntry): string {
   ...
   */
  private uptimeToMinutes(uptime: string): number {
    let minutes = 0;
    const days = uptime.match(/(\d+)d/);
    const hours = uptime.match(/(\d+)h/);
    const mins = uptime.match(/(\d+)m/);

    if (days) minutes += parseInt(days[1], 10) * 60 * 24;
